...
!     AND C USING EQUATIONS TAKEN FROM CENTURY.
      ...
      CS=MIN(10.,SQRT(CDG(ISL,ISA)*SUT(ISL,ISA))*PRMT(70)*OX*X1)
    ...
    !     COMPUTE N/C RATIOS
      ...
      X1=1000.*(WNH3(ISL,ISA)+WNO3(ISL,ISA))/WT(ISL,ISA)
      IF(X1>7.15)THEN
          BMNC=.33
          HSNC=.083
          HPNC=.143
...
!     COMPARE SUPPLY AND DEMAND FOR N
      ...
      ELSE
          SUM=SUM+THPNP-PN9
      END IF
      WMIN=MAX(1.E-5,WNO3(ISL,ISA)+SUM)
...
!     ACTUAL TRANSFORMATIONS
      ...
      THPNA=THPNP*X3
      SGMN=SGMN+SUM
      RNMN(ISL,ISA)=SUM-DMDN
!     UPDATE
      IF(RNMN(ISL,ISA)>0.)THEN
          X1=PRMT(73)*RNMN(ISL,ISA)
          WNH3(ISL,ISA)=WNH3(ISL,ISA)+X1
          WNO3(ISL,ISA)=WNO3(ISL,ISA)+RNMN(ISL,ISA)-X1
      ELSE
	      X1=WNO3(ISL,ISA)+RNMN(ISL,ISA)
	      IF(X1>0.)THEN
	          WNO3(ISL,ISA)=X1
	      ELSE
	          RNMN(ISL,ISA)=-WNO3(ISL,ISA)
	          WNO3(ISL,ISA)=1.E-10
	      END IF
      END IF
      DF1=TLSNA
      ...
      DF5=THPNA-HPNC*X1
      DF6=XZ-WNO3(ISL,ISA)-WNH3(ISL,ISA)
      ...
      RSPC(ISL,ISA)=MAX(0.,.3*TLSLCA+A1CO2*(TLSLNCA+TLMCA)+ABCO2*TBMCA+ASCO2*&
      THSCA+APCO2*THPCA)
...
!         SLAP=SLAP+VAR(103,ISA)
      ...
      AD2=WLSC(ISL,ISA)+WLMC(ISL,ISA)+WBMC(ISL,ISA)+WHSC(ISL,ISA)+WHPC(ISL,ISA)
      !AD2=WNO3(ISL,ISA)+WNH3(ISL,ISA)+WHSN(ISL,ISA)+WHPN(ISL,ISA)+WNOU&
      !(ISL,ISA)+WLSN(ISL,ISA)+WLMN(ISL,ISA)+WBMN(ISL,ISA)
      DF=AD1-AD2-RSPC(ISL,ISA)
      IF(ABS(DF)>.1)WRITE(KW(1),'(A,4I4,20E16.6)')'#####',IY,MO,KDA,ISL,&
      AD1,AD2,RSPC(ISL,ISA),DF
      RETURN
      END