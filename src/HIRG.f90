...
!     MANUAL OPTION.  EROSION AND RUNOFF ARE ESTIMATED.
      ...
      IF(IRY==0)THEN
          ...
          SELECT CASE(IAC(ISA))
              ...
              CASE(2)    
                  !Irrigate when ponding depth goes below the minimum depth in the paddy jaehak jeong 2016
                  IF(PADDY_STO(1,ISA)<PADDY_HMIN(ISA))X4=AVIR-PADDY_STO(1,ISA)
          END SELECT
      ELSE    
          ! MANUAL OR LAGOON
          IF(IAC(ISA)==0)THEN
      ...
      IF(AVIR>ARMN(ISA))THEN      
          ...
          if(ISALT>0) then
          do m=1,mion
            irrsalt(ISA,m) = .01*X4*irrig_csalt(ISA,m) !kg/ha
            vsalt(ISA,m) = irrsalt(ISA,m) !kg/ha 
          enddo
          endif
