...
!     & HARVESTING.
      ...
      DO K=1,NN
          I2=LY(IRO(ISA),K,ISA)
          IF(JH(IRO(ISA),KT(ISA),ISA)==KDC(I2))EXIT
      END DO
      IF(K>NN.OR.KGO(I2,ISA)>0)GO TO 26
      ZX=0.
      DO I=1,NBSL(ISA)
          ISL=LID(I,ISA)
          Z1=Z(ISL,ISA)
...
!	  TCPA(I2)=TCPA(I2)+WSA(ISA)
    ...
    6 EE=EMX(JT1)
      IF(II==NHC(19))THEN
          ...
          END DO
          IF(IHD<=NHRD(IOW))EE=EE*GCOW(IHD,ISA)/WSAX
      END IF
   ...
   10 CALL PESTF
      X6=PSTF(ISA)
      TPSF(ISA)=TPSF(ISA)+X6
      NPSF(ISA)=NPSF(ISA)+1
      IF(II==NHC(19).AND.LGZ==0)THEN
          RST0(ISA)=RSTK(IRO(ISA),KT(ISA),ISA)
          CALL TGRAZ(JRT)
          IF(JRT==0)GO TO 6
          RETURN
      END IF
      ...
      ELSE
          ...
          IF(II/=NHC(1))THEN
              ...
              STDK(JJK,ISA)=MAX(1.E-10,STDK(JJK,ISA)-Y6)
              ...
              YLK=MIN(.9*(UK1(JJK,ISA)+STDK(JJK,ISA)),YLD(JJK)*X8+Z4)
              X10=X4-YLN+Y4
