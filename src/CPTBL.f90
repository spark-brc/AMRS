...
!     THIS SUBPROGRAM READS CROP TABLE TO DETERMINE CROP PARAMETERS
      ...
      IF(L>LC)THEN
          ...
          !           9 FOR COTTON
          !          10 FOR LEGUME TREES
          !40/ FRST(= FROST DAMAGE PARMS--NUMBERS BEFORE DECIMAL = TMN(DEG C1,2)
          !           NUMBERS AFTER DECIMAL = FRACTION YLD LOST WHEN GIVEN TMN
          !           IS EXPERIENCED.
          ...
          !57  EXTC = LIGHT LIGHT EXTINCTION COEFFICIENT
          READ(KR(4),'()')
          READ(KR(4),'()')
          J2=-10
          DO WHILE(J2/=JX(6))
...
!	      IF(YTP(63)>0)EXTC(LC)=YTP(63)
      ...
      IF(IDC(JJK)==NDC(7).OR.IDC(JJK)==NDC(8).OR.IDC(JJK)==NDC(10))THEN
          IF(XMTU(JJK)<1.)XMTU(JJK)=30.
          PHU(JJK,IHU(JJK,ISA),ISA)=CAHU(1,365,TBSC(JJK),0)*XMTU(JJK)
          XMTU(JJK)=(1.-EXP(-FTO(JJK)/XMTU(JJK)))/144.
      END IF
      Y1=PPLP(1,JJK)
      Y2=PPLP(2,JJK)
      IF(Y2>Y1)THEN
	  ...
	  POPX(JJK)=X3
      IF(IDC(JJK)==NDC(3).OR.IDC(JJK)==NDC(6))IRP(ISA)=1
	  RETURN
    5 FORMAT(5X,'!!!!! PLANT POP DID NOT CONVERGE')
 3420 FORMAT(1X,I4,1X,A4,57F8.0)
      END