...
!     THIS SUBPROGRAM CONTROLS ROUTING OPERATIONS.
      ...
      !rtb salt: copy upstream loads into current stream, to prepare for routing
      if(ISALT>0) then
      do m=1,mion
        surfqsalt(IDO,m) = surfqsalt(IDN1,m)
        gwsalt(IDO,m) = gwsalt(IDN1,m)
        latsalt(IDO,m) = latsalt(IDN1,m)
        surfsalt(IDO,m) = surfsalt(IDN1,m)
        qrfsalt(IDO,m) = qrfsalt(IDN1,m)
      enddo
      endif
