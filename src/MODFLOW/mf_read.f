...
C     Open name file
      FNAME = 'MODFLOW/modflow.mfn' !this is required
...
!     Allocate a few variablesfor MODFLOW since it's subroutine (LMT7BAS7AR) isn't called, tcw
...
C     Initialize daycount (for mf_read subroutine) and read_stress flag
      ...
      if(mf_obs_flag.eq.1) then
        open(30050,file='MODFLOW/modflow.obs')
        read(30050,*)
        ...
        if(num_MF_obs.gt.0) then
          ...
          enddo
          open(30051,file='MODFLOW/amf_MODFLOW_obs_head')
