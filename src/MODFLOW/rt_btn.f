...
C       Open up file to output XY (surface contour) concentration at each output time
        OPEN(ICONC+I,FILE='MODFLOW/amf_RT3D_CONC'//COMP,
     &     STATUS='UNKNOWN')
      ENDDO   
...
*     Allocate general arrays for the simulation
      ...
      allocate(ICBUND(NCOL,NROW,NLAY,10))
      ...
      allocate(CNEW(NCOL,NROW,NLAY,10))
      allocate(COLD(NCOL,NROW,NLAY,10))
      allocate(CWGT(NCOL,NROW,NLAY,10))
      allocate(PRSITY(NCOL,NROW,NLAY))
      allocate(RETA(NCOL,NROW,NLAY,10))
      allocate(CADV(NCOL,NROW,NLAY,10))
      allocate(CSR(NCOL,NROW,NLAY,10))
...
C		Create and open up files for observation cell concentration output        
		DO INDEX=1,NCOMP
		  FLNAME = 'MODFLOW/amf_RT3D_OBS'//SPECIES(INDEX)
...
C         ASCII output
		  IF (WRITEFLAG.EQ.0 .OR. WRITEFLAG.EQ.2) THEN  
		  DO I=1, NROW
            WRITE(ICONC+ICOMP,1082) (CNEW(J,I,K,ICOMP),j=1,ncol)
		  ENDDO
		  ENDIF
	  ENDDO
 ...
 1081 format(1000(e16.7))
 1082 format(1000(E14.4))
