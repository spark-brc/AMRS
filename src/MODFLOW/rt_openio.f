...
*     Open up the file containing the input and output file names      
      OPEN(FNAMES,FILE='MODFLOW/rt3d_filenames',STATUS='OLD')
...
*     Read (from the BTN file) the packages that are used for this simulation
      ...
      !open up salt file, if package has been enabled
      if(trnop(6)) then
	  READ(FNAMES,*) FN10
        open(insalt, file=FN10, status='old')
      endif

      OPEN(IOUT,FILE='MODFLOW/amf_RT3D_out',STATUS='UNKNOWN')
