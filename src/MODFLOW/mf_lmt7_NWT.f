...
C
      SUBROUTINE LMT7BAS7AR(INUNIT,IGRID)
C **********************************************************************
...
C     Record saturated thickness array for use by UZF-RT3D, rtb
      if (rt_active.eq.1) DH = BUFF
...
!!     Record QXX array for use by UZF-RT3D
      if (rt_active.eq.1) QX = BUFF
...
C     Record QYY array for use by UZF-RT3D, rtb
      if (rt_active.eq.1) QY = BUFF
...
C     Record QZZ array for use by UZF-RT3D, rtb
      if (rt_active.eq.1) QZ = BUFF
...
C     Record storage array for use by UZF-RT3D, rtb
      if (rt_active.eq.1) QSTO = BUFF
        ...
        DO I=1,NROW
          DO J=1,NCOL
            ...
            ELSEIF(ILMTFMT.EQ.1) THEN
              WRITE(IUMT3D,*) K,I,J,BUFF(J,I,K)              
            ENDIF
            !record for use by UZF-RT3D, rtb
            if(rt_active.eq.1) then
              CNH(count,1) = K
              CNH(count,2) = I
              CNH(count,3) = J
              CNH(count,4) = BUFF(J,I,K)
              count = count + 1
            endif
          ENDDO
        ENDDO
...
C     Record QXX array for use by UZF-RT3D, rtb
      if (rt_active.eq.1) QX = BUFF
...
C--WRITE WELL LOCATION AND RATE ONE AT A TIME
      rt_nwel = NWELLS !for use by UZF-RT3D, rtb
      FWEL = .TRUE.
      count = 1
      DO L=1,NWELLS
        IL=WELL(1,L)
...
C--FOR EACH DRAIN ACCUMULATE DRAIN FLOW
      rt_ndrn = NDRAIN !for use by RT3D, rtb
      FDRN = .TRUE.
      count = 1
      DO L=1,NDRAIN
C
...
C--FOR EACH RIVER REACH ACCUMULATE RIVER FLOW
      rt_nriv = NRIVER !for use by UZF-RT3D, rtb
      FRIV = .TRUE.
      count = 1
      DO L=1,NRIVER
C
...
C     Record recharge array for use by UZF-RT3D, rtb
      if (rt_active.eq.1) RT_IRCH = IRCH
      if (rt_active.eq.1) RT_RECH = RECH
      if (rt_active.eq.1) FRCH = .TRUE.
...
C     Record ET array for use by UZF-RT3D, rtb
!      if (rt_active.eq.1) then
!        FEVT = .TRUE.
!        RT_IEVT = IEVT
!        DO IR=1,NROW
!          DO IC=1,NCOL
!            RT_EVTR(IC,IR) = BUFF(IC,IR,1)
!          ENDDO
!        ENDDO
!      endif
...
C--FOR EACH GENERAL HEAD BOUND ACCUMULATE FLOW INTO AQUIFER
      rt_nghb = NBOUND !for use by UZF-RT3D, rtb
      FGHB = .TRUE.
      count = 1
      DO L=1,NBOUND
C
...
C     Record water content array for use by UZF-RT3D, rtb
      if (rt_active.eq.1) MC = BUFF
...
C     Record water content array for use by SWAT-MODFLOW and UZF-RT3D, rtb, tcw
      UZET = BUFF !tcw
...
C     Record water content array for use by UZF-RT3D, rtb
      if (rt_active.eq.1) RT_GWET = BUFF !rtb
