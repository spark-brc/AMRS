*     This subroutine runs the MODFLOW simulator for the time period indicated
*     This subroutine is called for each day of the APEX simulation. 
...
*     Declare variables
      ...
      if (read_stress.eq.1) then
        ...
        IF(IUNIT(4).GT.0) CALL GWF2RIV7RP(IUNIT(4),IGRID) !River stage is provided by APEX
        IF(IUNIT(5).GT.0) CALL GWF2EVT7RP(IUNIT(5),IGRID) !Remaining ET is provided by APEX
        IF(IUNIT(7).GT.0) CALL GWF2GHB7RP(IUNIT(7),IGRID) !General Head
        IF(IUNIT(8).GT.0) CALL GWF2RCH7RP(IUNIT(8),IGRID) !recharge by is provided by APEX
     ...
     &                   CALL GWF2HYD7SFR7RP(IUNIT(43),KKPER,IGRID)
        IF(IUNIT(55).GT.0)CALL GWF2UZF1RP(IUNIT(55),KKPER,
     &                                   IUNIT(44),IGRID)
     ...
     &                            IGRID)    
      ...
      !pass APEX information to MODFLOW
      if(iunit(4).gt.0) call amrt_mfRiver !River package
      if(iunit(8).gt.0) call amrt_recharge !Recharge package
      if(iunit(5).gt.0) call amrt_evt !EVT package

      !If RT3D is used, read in stress information 
      if (rt_active.eq.1) then
        call amrt_rt3d_stress
      endif
      ...
      call units(perlen_day, ITMUNI, 4, 1, 1, MF_leapyr)
...
C--SWM: SWAP POINTERS FOR LMT DATA TO CURRENT GRID
        !populate the variables - if SM or AMRT is used, this is handled by amrt_read_river2grid subroutine, !tcw
 ...
 9876   FORMAT(/1X,'SAVING SATURATED THICKNESS AND FLOW TERMS ON UNIT',
   ...
   87         FORMAT(1X,'FAILURE TO MEET SOLVER CONVERGENCE CRITERIA',/
     &       1X,'BUDGET PERCENT DISCREPANCY IS',F10.4)
        ...
        !If solute transport is simulated, call RT3D subroutines to simulate reactive transport for the current flow time step
        if(rt_active.eq.1) then
          call amrt_rt3d_run
        endif
...
*     for the current stress period.
      ...
      if(mf_obs_flag.eq.1) then
        if(num_MF_obs.gt.0) then
          ...
          enddo
          write(30051,101) (obs_heads(i),i=1,num_MF_obs)
        endif
      endif
