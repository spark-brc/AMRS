...
*     the SWAT step size)
      ...
      do ntrans=1,5000
        ...
        !compute change in concentration due to kinetic chemical reactions
        if(trnop(4)) call rct_solve

        !compute change in salt ion concentration due to equilibrium chemistry reactions
        if(trnop(6)) then
          call salt_solve
        endif
...
100   call units(dt, ITMUNI, 4, 1, 1, MF_leapyr) !tcw
      ...
      amrt_csolute_tot_mo = amrt_csolute_tot_mo + cnew
      amrt_csolute_avg_mo = amrt_csolute_tot_mo / day_count_mo

      !calculate average recharge for the current year (rtb avg)
      amrt_csolute_tot_yr = amrt_csolute_tot_yr + cnew
      amrt_csolute_avg_yr = amrt_csolute_tot_yr / day_count_yr
        ...
        do j=1,ncol
          do i=1,nrow
            if(ICBUND(j,i,k,1).eq.0 .or. HNEW(j,i,k).eq.HDRY) then
              do icomp=1,ncomp
                amrt_csolute_avg_mo(j,i,k,icomp) = 0.
                amrt_csolute_avg_yr(j,i,k,icomp) = 0.  
              enddo
            endif
          enddo
        enddo
