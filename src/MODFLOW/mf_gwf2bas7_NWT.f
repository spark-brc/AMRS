...
!        USE MODEVENT 
        ...
        logical :: readMFinput, mf_ran !tcw
        integer :: MF_leapyr, mf_interval !tcw
        !type(event), target :: OnRivPkg, OnUZFPkg, OnRechPkg, MFrunTop !aqd  !Ali
        !type(event), target :: OnRT3Dinit, OnRT3D !tcw  !Ali
        PARAMETER(NIUNIT=100)
        character(len=4), dimension(:), allocatable :: CUNIT
        integer, dimension (:), allocatable :: IBDT
        integer :: kkper,kper,daycount,daytotal,read_stress,kstp
        INTEGER, SAVE, POINTER    ::NCOL,NROW,NLAY,NPER,NBOTM,NCNFBD
        INTEGER, SAVE, POINTER    ::ITMUNI,LENUNI,IXSEC,ITRSS,INBAS
        INTEGER, SAVE, POINTER    ::IFREFM,NODES,IOUT,MXITER,IRESTART
        INTEGER, SAVE, POINTER    ::KPERSTART,KSTPSTART,IUNITSTART
...
C  this file to use the IMPLICIT NONE statement.
      ...
      SUBROUTINE GWF2BAS7AR(INUNIT,VERSION,IUDIS,IUZON,IUMLT,
...
C2------Open all files in name file.
      CALL SGWF2BAS7OPEN(INUNIT,IOUT,IUNIT,NIUNIT,
...
C8E-----READ BOUNDARY ARRAY(IBOUND).
  ...
  280    CONTINUE
      ...
      !Record the number of constant-head cells, for use by UZF-RT3D, rtb
      maxcnh = 0
      do K=1,NLAY
        do I=1,NROW
          do J=1,NCOL
...
C4------"PERIOD", "HEAD", "DRAWDOWN", OR "COMPACT".
  ...
  100     FORMAT(1X,/1X,'RESTART OPTION ACTIVE. RESTART AT PERIOD ',I5,
     +          'STEP ',I5)
