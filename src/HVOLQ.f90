...
!     GIVEN DAILY PRECIPITATION AND SNOW MELT.
      ...
      IF(PADDY_HWEIR(ISA)>0)THEN 
          ...
          !PADDY_STO(3,ISA)=PADDY_STO(3,ISA)+(RFQN(ISA)+QN(ISA))/WSA(ISA) !minN kg/ha
      ...
      IF(RFV(IRF(ISA))>0.)THEN
          ...
          SELECT CASE(ITYP)
              ...
              CASE(5)
                  ...
                  !TPK=.1*DURE+TC(IDO)
                  TPK=TC(IDO)
                  !QPQX=MAX(.03,MIN(.5417*RTO-.0042,.465))
                  RTO=10.*RTO
                  QPQX=RTO/(RTO+EXP(SCRP(30,1)-SCRP(30,2)*RTO))
                  !RQRB(IDO)=2.*QPQX*QVOL(IDO)/TPK
                  RQRB(IDO)=MIN(.9*REP,2.*QPQX*QVOL(IDO)/TPK)
                  X1=.75*QVOL(IDO)
          END SELECT        
          QPR(IDO)=X1/X4
