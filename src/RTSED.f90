...
!     CONC OF REACH INFLOW, VEGETATIVE COVER, AND CHANNEL ERODIBILITY.
      ...
      DO J=1,12
          ...
          ADD=ADD+UP1(J,IDN2)
      END DO
      TP1=WSA2*(WPML(LD1,IDN2)+WPO(LD1,IDN2)+WPOU(LD1,IDN2)+TOT+&
      STDOP(IDN2)+ADD)
      !AD1=0.
      !DO I=1,NBSL(IDN2)
          !ISL=LID(I,IDN2)
   ...
   34 IF(QQ<1.E-5)THEN
      ...
      DO IT=1,10
          ...
          FU=Q-QQ
          X6=MAX(1.,QQ)
           IF(ABS(FU/X6)<.01)THEN
            IF(IMF>0) dep_chan(NBSA(IDN2)) = DCH !rtb channel depth for the subarea, to use in River package 
            EXIT
          ENDIF
   ...
   10 VAV=MAX(.1,(X1+VCH*QQ)/QAV)
      ...
      !rtb salt - add up total salt ion mass (kg)
      if(ISALT>0) then
      do m=1,mion
      ...
      DO J=1,12
          ...
          ADD=ADD+UP1(J,IDN2)
      END DO
      TP2=WSA2*(WPML(LD1,IDN2)+WPO(LD1,IDN2)+WPOU(LD1,IDN2)+TOT+&
      STDOP(IDN2)+ADD)
     ...
     IF(IMF>0)THEN
         !rtb MODFLOW: calculate mineral P concentration in the reach
         if(QVOL(IDO).gt.100) then
           p_chan(NBSA(IDN2)) = (VARH(19,IDO)*1000.) / QVOL(IDO) !g/m3
         else
           p_chan(NBSA(IDN2)) = 0.
         endif
     ENDIF
